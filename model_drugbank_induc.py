...
class HOPGCNII_DDI(nn.Module):
    ...
    def forward(self, triples):
        ...
        for i in range(len(self.blocks)):
            h_feaures.append(F.normalize(h_graphs[i] + h_data_mol2vec))
            t_feaures.append(F.normalize(t_graphs[i] + t_data_mol2vec))
...
class HOPGCNII_DDI_Block(nn.Module):
    ...
    def forward(self, h_data, t_data, b_graph, h_data_edge, t_data_edge):
        ...
        h_graph = h_graph_gat+h_gcn+h_edge #[B₁, 128]
        t_graph = t_graph_gat+t_gcn+t_edge#[B₁, 128]
