...
def load_features(path):
    df = pd.read_csv(path)
    id_col = None
    for candidate in  ['id', 'drug_id', 'drugbank_id'] :
        if candidate in df.columns:
            id_col = candidate
            break
    if id_col is None:
        raise ValueError(f"No ID column found in {path}")
    feature_columns = [col for col in df.columns if col != id_col]
    feature_dict = {}
    for _, row in df.iterrows():
        drug_id = row[id_col]
        try:
            feature_array = row[feature_columns].astype(np.float32).values
            feature_tensor = torch.tensor(feature_array, dtype=torch.float32)
        except Exception as e:
            print(f"[Error] drug_id: {drug_id} has invalid features. Error: {e}")
            continue
        feature_dict[drug_id] = feature_tensor
    return feature_dict
...
mol2vec_feature_path = config[dp_dataset_name]["mol2vec_path"]  # 假设 config 中有这个字段
molT5_feature_path = config[dp_dataset_name]["molT5_feature_path"]
id_molT5=load_features(molT5_feature_path)
id_mol2vec = load_features(mol2vec_feature_path)
...
class DrugDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        if 'drugbank' not in dp_dataset_name:
            for h, t, r in batch:
                if r == 1:
                    ...
                    pos_h_mol2vec.append(h_data.mol2vec)
                    pos_t_mol2vec.append(t_data.mol2vec)
                    pos_h_molT5.append(h_data.molT5)
                    pos_t_molT5.append(t_data.molT5)
                ...
                if r == 0:
                    ...
                    neg_h_mol2vec.append(h_data.mol2vec)
                    neg_t_mol2vec.append(t_data.mol2vec)
                    neg_h_molT5.append(h_data.molT5)
                    neg_t_molT5.append(t_data.molT5)
        else:
            for h, t, r in batch:
                pos_rels.append(r)
                h_data = self.__create_graph_data(h)
                ...
                for neg_h in neg_heads:
                    neg_h_data = self.__create_graph_data(neg_h)
                    neg_t_data = t_data
                    ...
                    neg_h_mol2vec.append(neg_h_data.mol2vec)
                    neg_t_mol2vec.append(neg_t_data.mol2vec)
                    neg_h_molT5.append(neg_h_data.molT5)
                    neg_t_molT5.append(neg_t_data.molT5)
                ...
                for neg_t in neg_tails:
                    neg_h_data = h_data
                    neg_t_data = self.__create_graph_data(neg_t)
        ...
        pos_h_mol2vec = torch.stack(pos_h_mol2vec)
        pos_t_mol2vec = torch.stack(pos_t_mol2vec)
        pos_h_molT5 = torch.stack(pos_h_molT5)
        pos_t_molT5 = torch.stack(pos_t_molT5)
        # pos_edge
        pos_h_edge_samples = Batch.from_data_list(pos_h_edge_samples)
        pos_t_edge_samples = Batch.from_data_list(pos_t_edge_samples)

        
        pos_tri = (pos_h_samples, pos_t_samples, pos_rels, pos_b_samples, pos_h_edge_samples, pos_t_edge_samples, pos_h_mol2vec, pos_t_mol2vec,
            pos_h_molT5, pos_t_molT5,)
        ...
        neg_h_mol2vec = torch.stack(neg_h_mol2vec)
        neg_t_mol2vec = torch.stack(neg_t_mol2vec)
        neg_h_molT5 = torch.stack(neg_h_molT5)
        neg_t_molT5 = torch.stack(neg_t_molT5)
        # neg_edge
        neg_h_edge_samples = Batch.from_data_list(neg_h_edge_samples)
        neg_t_edge_samples = Batch.from_data_list(neg_t_edge_samples)

        neg_tri = (neg_h_samples, neg_t_samples, neg_rels, neg_b_samples, neg_h_edge_samples, neg_t_edge_samples, neg_h_mol2vec, neg_t_mol2vec,
            neg_h_molT5, neg_t_molT5)
    ...
    def __create_graph_data(self, id):
        ...
        data = Data(x=n_features, edge_index=edge_index, edge_attr=edge_attr)
        data.mol2vec = id_mol2vec.get(id, torch.zeros(300))
        data.molT5 = id_molT5.get(id, torch.zeros(300))
        return data
