...
from model_drugbank_trans import HOPGCNII_DDI
import custom_loss
import AdaptiveMarginSigmoidLoss
...
device = 'cuda:3' if torch.cuda.is_available() and params['use_cuda'] else 'cpu'
...
def set_seed(seed=42):
    ...
    torch.cuda.manual_seed_all(seed)    # 多卡随机数
    torch.backends.cudnn.deterministic = True  # 使卷积算法确定
    torch.backends.cudnn.benchmark = False     # 禁用自动选择最优算法
set_seed(42)
...
set_seed(42)
model = HOPGCNII_DDI([n_atom_feats, 300, 1024], 17, kge_dim, kge_dim, rel_total, [64, 64, 64, 64], [2, 2, 2, 2], 64,
                         0.0)

#loss = custom_loss.SigmoidLoss()
loss=AdaptiveMarginSigmoidLoss.AdaptiveMarginSigmoidLoss()
